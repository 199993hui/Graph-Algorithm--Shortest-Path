...
# ==============DETECT CYCLE by Koo Xi Moon===================================
    ...
    def CheckCycle(self):
        ...
        for city in self.city:
            if visited[city] == False:
                if self.FindCycle(city, visited, stack, edges):
                    self.DisplayCycle(edges)
                    return True
        return False
    
    # Depth First Search
    def FindCycle(self, source, visited, stack, edges):
        visited[source] = True
        stack[source] = True
        #to find adjacent nodes
        neighbours = []
        items = self.graph[source,:]
        for i in range(5):
            if items[i] != 0:
                neighbours.append(i)
        ...
        for neighbour in neighbours:
            if visited[neighbour] == False:
                if self.FindCycle(neighbour, visited, stack, edges):
    ...
    # Depth First Search
    def FindCycleWithCertainNode(self, source, visited, edges, node):
        visited[source] = True
        #to find adjacent nodes
        neighbours = []
        items = self.graph[source,:]
        ...
        for neighbour in neighbours:
            if visited[neighbour] == False:
                if self.FindCycleWithCertainNode(neighbour, visited, edges, node):
                    edges.append([source,neighbour])
                    return True
            # determinant
    ...
    def GetCycleWithAppointedNode(self, node):
        ...
        while(self.FindCycleWithCertainNode(node, visited, edges, node)==False):
            self.AddEdge()
...
class Menu:
    ...
    def UserMenu(self):
        ...
        while(proceed == True):
            ...
            print("\t===================Menu===================")
            print("\t   1. Problem 1 - Strong Connection")
            print("\t   2. Problem 2 - Cycle Detection")
            print("\t   3. Problem 3 - Shortest Path")
            print("\t   4. Remove edges")
    ...
    def Function_3(self):
        ...
        if(sc in self.G.city.values() and dc in self.G.city.values()):
            if(self.G.check_reachable(sc,dc) == False):
                print("\n There is no path from " + sc + " to " + dc + ".")
                print("\n New graph generated...")
                # Random generate edges between cities until there is a path between the two vertices        
                while self.G.check_reachable(sc,dc) == False:
                    self.G.AddEdge() 
            self.G.DisplayGraph("Current Graph")
            self.G.ShortestPath(sc,dc)
            print("");
        else:
